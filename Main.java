...
public class Main {
    public static void internetOrder(OrderManager orderManager){
        ...
        for (String key: list.keySet()){
            System.out.println("Order №" + i+ "(" + key + "): " +
        ...
        for (int i=0;i<orderManager.tableCount;i++){
            if (orderManager.getOrder(i)!=null)
                System.out.println("Table "+ i + ": "+
                        orderManager.getOrder(i).toString());
        }
    }
    public static void addOrder(OrderManager orderManager, boolean flag){
        ...
        if (!flag) {
            ...
            switch(choose){
                case 1:
                    orderManager.addItem(address, new
                            Drink(temp,temp1,temp2));
                    break;
                case 2:
                    orderManager.addItem(address, new
                            Dish(temp,temp1,temp2));
                    break;
                default:
                    break;
            }
        ...
        while(true){
            ...
            switch(x){
                ...
                case 1:
                    ...
                    switch(choose){
                        case 1:
                            RestaurantOrder restaurantOrderDrink = new
                                    RestaurantOrder(2);
                            restaurantOrderDrink.add(new
                                    Drink(temp,temp1,temp2));
                            orderManager.add(restaurantOrderDrink, table);
                            break;
                        case 2:
                            RestaurantOrder restaurantOrderDish = new
                                    RestaurantOrder(2);
                            restaurantOrderDish.add(new
                                    Dish(temp,temp1,temp2));
